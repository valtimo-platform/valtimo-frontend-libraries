...
@Component({
  selector: 'valtimo-object-management-search-fields',
  templateUrl: './object-management-search-fields.component.html',
  styleUrls: ['./object-management-search-fields.component.scss'],
})
export class ObjectManagementSearchFieldsComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  moveRow(
    ...
    if (multipleSearchFields && moveUp && searchFieldIndex > 0) {
      ...
      this.updateSearchFields(objectId, filteredSearchFields);
  ...
  deleteSelectedSearchField(
    ...
    this.documentService
      .deleteDocumentSearch('bezwaar', selectedSearchField.key)
      .subscribe(
        () => {
          this.enableInput();
          this.hideModal();
  ...
  saveSearchField(objectId: string): void {
    this.disableInput();
    ...
    this.formData$.pipe(take(1)).subscribe(formData => {
      ...
      if (this.searchFieldActionTypeIsAdd) {
        this.documentService.postDocumentSearch('bezwaar', mappedFormData).subscribe(
          () => {
            this.enableInput();
            this.hideModal();
            this.refreshSearchFields();
        ...
        this.updateSearchFields(objectId, filteredFields);
      }
    });
  }

  updateSearchFields(objectId: string, newSearchFields: Array<SearchField>): void {
    this.disableInput();

    this.documentService.putDocumentSearch('bezwaar', newSearchFields).subscribe(
      () => {
        this.enableInput();
        this.hideModal();
        this.refreshSearchFields();
  ...
  private setDownload(objectId: string, searchFields: Array<SearchField>): void {
