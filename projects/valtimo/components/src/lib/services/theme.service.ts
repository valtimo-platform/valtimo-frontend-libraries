/*
 * Copyright 2015-2024 Ritense BV, the Netherlands.
 *
 * Licensed under EUPL, Version 1.2 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 * https://joinup.ec.europa.eu/collection/eupl/eupl-text-eupl-12
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" basis,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

// theme-preference.service.ts
import { Injectable } from '@angular/core';
import { BehaviorSubject, Observable } from 'rxjs';

@Injectable({
  providedIn: 'root'
})
export class ThemeService {
  private themeSubject: BehaviorSubject<string>;
  public readonly CDS_THEME = {
    G10: 'g10',
    G90: 'g90',
    SYSTEM: 'system'
  };
  public theme$: Observable<string>;

  constructor() {
    const initialTheme = this.CDS_THEME.G10;
    this.themeSubject = new BehaviorSubject<string>(initialTheme);
    this.theme$ = this.themeSubject.asObservable();
    this.setTheme(initialTheme);
    const colorSchemeQueryList = window.matchMedia('(prefers-color-scheme: dark)');
    this.updateSystemTheme(colorSchemeQueryList.matches);
    colorSchemeQueryList.addEventListener('change', (e: MediaQueryListEvent) => {
      this.updateSystemTheme(e.matches);
    });
  }

  setTheme(selectedTheme: string): void {
    if (selectedTheme !== this.CDS_THEME.SYSTEM) {
      document.documentElement.setAttribute('data-carbon-theme', selectedTheme);
    }
    else {
      this.themeSubject.next(selectedTheme);
      this.updateSystemTheme(true);
    }
    this.themeSubject.next(selectedTheme);
  }

  private updateSystemTheme(matches: boolean): void {
    const selectedTheme = this.themeSubject.getValue();
    if (selectedTheme === this.CDS_THEME.SYSTEM) {
      if (window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches) {
        document.documentElement.setAttribute('data-carbon-theme', 'g90');
      }
      else {
        document.documentElement.setAttribute('data-carbon-theme', 'g10');
      }
    }
  }
}
