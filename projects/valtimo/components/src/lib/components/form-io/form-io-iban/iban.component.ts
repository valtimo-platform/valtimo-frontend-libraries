...
export class FormIoIbanComponent implements FormioCustomComponent<any>, AfterViewInit, OnDestroy {
  ...
  public ngAfterViewInit(): void {
    setTimeout(() => {
      ...
      this._subscriptions.add(this.ibanForm.valueChanges.subscribe(() => {
        this.onValueChange();
      }));
    });
  }
  ...
  private onValueChange(): void {
    (this.value as any) = this.ibanForm.valid ? this.ibanForm.controls.iban.value : [this.ibanForm.value];
