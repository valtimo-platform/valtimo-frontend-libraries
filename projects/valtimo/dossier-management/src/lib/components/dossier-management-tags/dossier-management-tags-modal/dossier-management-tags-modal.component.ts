...
import {StatusModalCloseEvent, TagsModalType} from '../../../models';
import {
  BehaviorSubject,
  combineLatest,
  map,
...
@Component({
  selector: 'valtimo-dossier-management-tags-modal',
  templateUrl: './dossier-management-tags-modal.component.html',
  styleUrls: ['./dossier-management-tags-modal.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class DossierManagementTagsModalComponent implements OnInit, OnDestroy {
  @Input() public set type(value: TagsModalType) {
  ...
  private readonly COLORS: TagsColor[] = [
    TagsColor.Red,
    TagsColor.Magenta,
    TagsColor.Purple,
    TagsColor.Blue,
    ...
    TagsColor.CoolGray,
    TagsColor.WarmGray,
    TagsColor.HighContrast,
    TagsColor.Outline,
  ];

  private readonly _selectedColor$ = new BehaviorSubject<TagsColor>(undefined);
    ...
    map(([selectedColor]) =>
      this.COLORS.map(color => ({
        ...
        content: this.translateService.instant(
          'interface.tagType.' + TagsUtils.getTagTypeFromTagsColor(color)
        ),
        color,
        tagType: TagsUtils.getTagTypeFromTagsColor(color),
      }))
    )
  );
  ...
  public colorDropdownChange(event: {
    ...
    const newColor = event?.item?.color as TagsColor;
  ...
  private resetForm(): void {
    ...
    this._selectedColor$.next(TagsColor.Blue);
    this.statusFormGroup.markAsPristine();
    this.resetEditingKey();
  }
  ...
  private getFormValue(): TagsStatus {
    return {
      ...
      color: this.color.value as TagsColor,
    };
  }
}
